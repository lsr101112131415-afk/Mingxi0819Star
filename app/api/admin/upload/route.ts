import { getJourneyState, imageStore, initialLocations, saveJourneyState, storeImage, type StoredPhoto } from "../../../lib/netlify-data";
...
export async function POST(request: Request) {
  ...
  if (!initialLocations.some((location) => location.id === locationId)) return Response.json({ error: "请选择有效地点" }, { status: 400 });
  ...
  let sortOrder = Math.max(-1, ...state.photos.filter((photo) => photo.locationId === locationId).map((photo) => photo.sortOrder)) + 1;
  const created: StoredPhoto[] = [];
  try {
    for (const file of files) {
      const id = crypto.randomUUID();
      const extension = file.type === "image/png" ? "png" : file.type === "image/webp" ? "webp" : "jpg";
      const photo: StoredPhoto = { id, locationId, objectKey: `${locationId}/${id}.${extension}`, filename: file.name.slice(0, 200), mimeType: file.type, size: file.size, sortOrder, createdAt: Date.now() };
